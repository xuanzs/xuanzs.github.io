document.addEventListener("DOMContentLoaded", function () {
    ...
    if (team) {
        ...
        db.collection("assignments").doc("team" + team)
            .onSnapshot((doc) => {
...
function highlightCorrectStations(teamId, tableButtons) {
    firebase.firestore().collection("stationSubmission")
    .onSnapshot(snapshot => {
...
function getTop5Teams(stationData) {
    ...
    const topPairs = Array.from({ length: MAX_PAIRS }, () => ({ team: null, baby: -Infinity }));
...
function showTop5PairsAndBabyForStation(stationLabel) {
    ...
    unsubscribeStationPopup = firebase.firestore().collection("stationSubmission")
        ...
        .onSnapshot(doc => {
            ...
            const top5Pairs = Array.from({ length: MAX_PAIRS }, () => ({ team: null, baby: -Infinity }));

            for (let row = 1; row <= MAX_ROWS; row++) {
                const rowData = data[`row${row}`];
                if (!rowData || !Array.isArray(rowData.team) || !Array.isArray(rowData.baby)) continue;
