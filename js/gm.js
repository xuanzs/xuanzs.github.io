...
function ensureOptionExistsAndSelect(select, value) {
    ...
    if (!hasOption) {
        ...
        if (/^team\d+$/.test(value)) {
            db.collection('assignments').doc(value).get().then((doc) => {
                if (doc.exists) {
                    const data = doc.data();
...
function fillBabyDrop(teamSelectElement) {
    ...
    const babySelectElement = parentRow.querySelector(`.bottom .s${pairClass.substring(1)}`);

    babySelectElement.innerHTML = '<option value=""></option>';

    db.collection('assignments').doc(selectedTeamId).get()
        .then((doc) => {
            if (doc.exists) {
                const data = doc.data();
...
function fillPreFilledValues() {
    db.collection('stationSubmission').doc(`station${station}`).get().then((doc) => {
        if (doc.exists) {
            const data = doc.data();
            console.log(data);
            let see = [];
...
async function submitBtn() {
    ...
    let existingData = {};
    try {
        const docSnap = await db.collection("stationSubmission")
            .doc(`station${station}`)
            .get();
        if (docSnap.exists) {
            ...
            if (!p.disabled && !s.disabled  && sValue !== "") {
                ...
                if (row > 1) {
                    ...
                    if (!isNaN(belowBaby) && parseFloat(sValue) <= belowBaby) {
                        alert(`In Row ${row} - Pair ${pair}: Baby must be greater than Row ${row - 1}`);
                        return;
                    }
                }
                ...
                if (row === 1 && parseInt(sValue) < 3) {
                    ...
                    if (aboveP && aboveS && aboveP.disabled && aboveS.disabled) {
                        ...
                        console.log(`Enabled Row ${row + 1} - Pair ${pair} because baby was < 3 in Row ${row}`);
                    }
                }
                ...
                try {
                    await db.collection("submissionHistory").doc(`station${station}`)
                        .collection("entries")
                        .add(submissionEntry);
                            ...
                            if (parseInt(currentS?.value) <= 2 && currentS.value === "") {
                                if (nextRowP && nextRowS) {
                                    ...
                                    if (h === pair && parseInt(currentS?.value) === 2) {
                                        console.log(`Enabled Row ${row + 1} - Pair ${pair} because baby was 2`);
                                    }
                                }
                            }
                        }
                ...
                if (row === 2 && parseInt(sValue) <= 2 && row + 1 <= MAX_ROWS) {
                    ...
                    if (row3P && row3S) {
                        ...
                        console.log(`Enabled Row 3 - Pair ${pair} because baby was 2 in Row 2`);
                    }
                }
...
async function removeBabyFromAssignment (teamId, babyValue) {
    try {
        ...
        if (docSnap.exists) {
            ...
            console.log(`Removed one occurence of baby ${babyValue} from team ${teamId}`);
        } else {
