...
(function (window) {
  ...
  function initStatusListener() {
    ...
    let shutdownStationId = null;

    // 1) Listen shutdown doc (optional override)
    db.collection("skills")
      .doc("shutdown")
      .onSnapshot(
        (doc) => {
          if (!doc.exists) {
            shutdownStationId = null; // no override
            return;
          }
          const data = doc.data() || {};
          // normalize to string to match your ids
          shutdownStationId =
            data.station != null ? String(data.station).trim() : null;

          console.log("Shutdown active on station:", shutdownStationId);

		  if (shutdownStationId && map[shutdownStationId]) {
			const space = map[shutdownStationId];
			updateElements(space, "1"); // force shutdown category
			console.log(`🔥 Shutdown updated directly: space ${space} → category 1`);
		  }
        },
        (err) => {
          console.error("Error fetching shutdown doc:", err);
          shutdownStationId = null;
        }
      );

    // 2) Listen gamemaster accounts + apply override if match
      ...
      .onSnapshot(
        (doc) => {
          ...
          Object.keys(accounts).forEach((accountKey) => {
            ...
            // base category by status
            let category = status === "vacant" ? "3" : "4";

            // ✅ override: if shutdown station matches this id
            if (shutdownStationId && id === shutdownStationId) {
              category = "1";

              console.log(`Station ${id} is SHUTDOWN → category 1`);
  ...
  function updateElements(space, category) {
    ...
    document
      .querySelectorAll(".list__item.is-category-header")
      .forEach((el) => el.classList.remove("is-category-header"));
    markCategoryHeaders();
  ...
  function sortListByCategoryLevelAndSpace() {
    ...
    const items = Array.from(
      list.querySelectorAll(".list__item[data-category]")
    );
  ...
  function showLevelSpaces() {
    ...
    spacesList.filter(
      (item) => item.values().level === selectedLevel.toString()
    );
